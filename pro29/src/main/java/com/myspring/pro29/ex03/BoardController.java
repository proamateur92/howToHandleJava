...
public class BoardController {
	...
	public ResponseEntity<String> modArticle(@PathVariable("articleNO") Integer articleNO, @RequestBody ArticleVO articleVO) {
		ResponseEntity<String> resEntity = null;
		try {
			logger.info("modArticle 메서드 실행");
			logger.info(articleVO.toString());
			resEntity=new ResponseEntity("MOD_SUCCESSED",HttpStatus.OK);
		} catch(Exception e) {
			resEntity = new ResponseEntity(e.getMessage(),HttpStatus.BAD_REQUEST);
		}
		return resEntity;
	...
	public ResponseEntity<String> removeArticle(@PathVariable("articleNO") Integer articleNO) {
		...
		try {
			logger.info("removeArticle 메서드 실행");
			logger.info(articleNO.toString());
			resEntity = new ResponseEntity("REMOVE_SUCCESSED",HttpStatus.OK);
		} catch(Exception e) {
			resEntity = new ResponseEntity(e.getMessage(),HttpStatus.BAD_REQUEST);
		}
		return resEntity;
